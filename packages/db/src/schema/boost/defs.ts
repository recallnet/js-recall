...
export const boostBonus = pgTable(
  ...
  (t) => [
    ...
    // Composite index for active boosts (most common query pattern)
    // Note: Partial index WHERE is_active = true can be added in migration for better performance
    index("boost_bonus_user_active_idx").on(t.userId, t.isActive, t.expiresAt),
