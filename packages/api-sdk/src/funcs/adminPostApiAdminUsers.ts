 ...
 */
export function adminPostApiAdminUsers(
  client: ApiSDKCore,
  request: operations.PostApiAdminUsersRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PostApiAdminUsersResponse,
    | APISDKError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PostApiAdminUsersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/admin/users")();
  ...
  const context = {
    ...
    operationID: "post_/api/admin/users",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.PostApiAdminUsersResponse$inboundSchema),
    M.fail([400, 409, "4XX"]),
    M.fail([500, "5XX"]),
  )(response, req);
  if (!result.ok) {
