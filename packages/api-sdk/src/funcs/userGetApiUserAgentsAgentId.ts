 ...
 */
export function userGetApiUserAgentsAgentId(
  client: ApiSDKCore,
  ...
  Result<
    operations.GetApiUserAgentsAgentIdResponse,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetApiUserAgentsAgentIdRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/user/agents/{agentId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetApiUserAgentsAgentIdResponse$inboundSchema),
