 ...
 */
export function voteGetApiUserVotes(
  client: ApiSDKCore,
  ...
  Result<
    operations.GetApiUserVotesResponse,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetApiUserVotesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/user/votes")();
  ...
  const context = {
    ...
    operationID: "get_/api/user/votes",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetApiUserVotesResponse$inboundSchema),
    M.fail([400, 401, "4XX"]),
    M.fail([500, "5XX"]),
  )(response);
  if (!result.ok) {
