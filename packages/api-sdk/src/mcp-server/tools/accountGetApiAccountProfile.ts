/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { accountGetApiAccountProfile } from "../../funcs/accountGetApiAccountProfile.js";
import { ToolDefinition, formatResult } from "../tools.js";

export const tool$accountGetApiAccountProfile: ToolDefinition = {
  name: "account-get-api-account-profile",
  description: `Get team profile

Get profile information for the authenticated team`,
  tool: async (client, ctx) => {
    const [result, apiCall] = await accountGetApiAccountProfile(client, {
      fetchOptions: { signal: ctx.signal },
    }).$inspect();

    if (!result.ok) {
      return {
        content: [{ type: "text", text: result.error.message }],
        isError: true,
      };
    }

    const value = result.value;

    return formatResult(value, apiCall);
  },
};
