...
export async function findUserAgentCompetitions(
  ...
  try {
    ...
    // Group by competition to avoid duplicates when multiple user agents are in same competition
    const uniqueCompetitions = new Map();
    results.forEach((data) => {
      if (data.competitions && !uniqueCompetitions.has(data.competitions.id)) {
        uniqueCompetitions.set(data.competitions.id, data.competitions);
      }
    });

    return {
      competitions: Array.from(uniqueCompetitions.values()),
      total: total[0]?.count || 0,
    };
  } catch (error) {
    console.error(
