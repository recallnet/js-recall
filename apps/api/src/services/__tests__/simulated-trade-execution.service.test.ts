...
vi.mock("@recallnet/services", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@recallnet/services")>();
  return {
    ...actual,
...
describe("SimulatedTradeExecutionService", () => {
  ...
  beforeEach(async () => {
    ...
    const tradeRepo = await import("@recallnet/db/repositories/trade");
    mockCreateTradeWithBalances = vi.mocked(tradeRepo.createTradeWithBalances);

    const constraintsRepo = await import(
      "@recallnet/db/repositories/trading-constraints"
    );
  ...
  describe("Trading Constraints", () => {
    ...
    it("should allow stablecoin trades to bypass constraints", async () => {
      ...
      const DexScreenerModule = await import("@recallnet/services");
