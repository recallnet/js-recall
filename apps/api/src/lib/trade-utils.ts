...
export function calculateSlippage(fromValueUSD: number): SlippageResult {
  if (!Number.isFinite(fromValueUSD) || fromValueUSD < 0) {
    throw new Error("fromValueUSD must be a finite, non-negative number");
  }
  ...
  const baseSlippage = Math.min(
    0.02 * (Math.log1p(fromValueUSD / 10000) / Math.LN10),
    MAX_SLIPPAGE,
  );

  // Apply randomness: ±10% variation
  const randomMultiplier = 0.9 + Math.random() * 0.2;
  // Note: defensive to ensure we don't get negative slippage
