...
class TransactionProcessor {
  ...
  async processTransaction(transaction: {
    ...
    try {
      ...
      await this.insertConvictionClaim(
        ...
        claimData.season,
        claimData.duration, // amount claimed based on duration
  ...
  private async insertConvictionClaim(
    ...
    try {
      ...
      if (saved) {
        this.#logger.info(
          `Inserted conviction claim: account=${claimer}, season=${season}, claimed_amount=${claimedAmount}` +
            `eligible_amount=${eligibleAmount}, duration=${duration}s in tx ${transactionHash}`,
        );
      } else {
        this.#logger.debug(
