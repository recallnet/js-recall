CREATE TABLE "epochs" (
	...
	CONSTRAINT "epochs_number_unique" UNIQUE("number")
);
...
CREATE TABLE "vote_assignments" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"amount" numeric(30, 18) NOT NULL,
	CONSTRAINT "vote_assignments_pkey" PRIMARY KEY("stake_id","user_id","epoch")
);
--> statement-breakpoint
...
CREATE TABLE "votes_performed" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "votes_performed_pkey" PRIMARY KEY("user_id","agent_id","epoch")
);
--> statement-breakpoint
...
CREATE INDEX "idx_rewards_epoch" ON "rewards" USING btree ("epoch");--> statement-breakpoint
CREATE INDEX "idx_rewards_address" ON "rewards" USING btree ("address");--> statement-breakpoint
CREATE UNIQUE INDEX "uq_rewards_roots_epoch" ON "rewards_roots" USING btree ("epoch");--> statement-breakpoint
CREATE INDEX "idx_rewards_tree_epoch_level_idx" ON "rewards_tree" USING btree ("epoch","level","idx");--> statement-breakpoint
...
CREATE INDEX "idx_stakes_address" ON "stakes" USING btree ("address");--> statement-breakpoint
CREATE INDEX "idx_vote_assignments_user_epoch" ON "vote_assignments" USING btree ("user_id","epoch");--> statement-breakpoint
CREATE INDEX "idx_vote_assignments_epoch" ON "vote_assignments" USING btree ("epoch");--> statement-breakpoint
CREATE INDEX "idx_votes_performed_agent_epoch" ON "votes_performed" USING btree ("agent_id","epoch");--> statement-breakpoint
