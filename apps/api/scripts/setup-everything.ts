...
async function setupEverything() {
  try {
    ...
    let adminUsername = await prompt(
      rl,
      "Enter admin username (default: admin): ",
    );
    adminUsername = adminUsername || "admin";

    let adminEmail = await prompt(
      rl,
    ...
    if (useGeneratedPassword.toLowerCase() !== "n") {
      ...
      if (!adminPassword) {
        console.log(
          "Password cannot be empty. Using a generated password instead.",
        );
        adminPassword = generateRandomPassword();
        console.log(`Generated password: ${adminPassword}`);
      }
    ...
    if (
      !serverOutput.includes("Server is running") &&
    ...
    try {
      ...
      if (apiKeyMatch && apiKeyMatch[1]) {
        ...
        console.log(
          `${colors.green}Successfully extracted admin API key!${colors.reset}`,
        );
      } else {
        console.warn(
    ...
    console.log(
      "Your Trading Simulator Server is now set up and ready to use.",
    );
    ...
    if (adminApiKey) {
      ...
      console.log(
        "API Key: [NOT CAPTURED - Please check command output above]",
      );
    }
    console.log("----------------------------------------\n");
