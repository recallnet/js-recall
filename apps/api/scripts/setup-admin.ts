  ...
  try {
    ...
    if (!password) {
      ...
      if (useGenerated.toLowerCase() !== "n") {
        ...
        console.log(
          `${colors.green}Generated password: ${password}${colors.reset}`,
        );
      } else {
        password = await promptPassword(
          `${colors.cyan}Enter admin password: ${colors.reset}`,
        );
        ...
        if (password !== confirmPassword) {
          console.error(
            `${colors.red}Passwords do not match. Please try again.${colors.reset}`,
          );
          rl.close();
          return;
        }
    ...
    console.log(
      `${colors.blue}Attempting to connect to server at ${baseUrl}...${colors.reset}`,
    );
    ...
    console.log(
      `${colors.red}Make sure to securely store this API key.${colors.reset}`,
    );
    console.log(
      "\nYou can now use this API key to authenticate admin requests:",
    );
    console.log(
      `${colors.blue}Authorization: Bearer ${adminApiKey}${colors.reset}`,
    );
    ...
    if (axios.isAxiosError(error) && error.response) {
      ...
      console.error(
        `\n${colors.red}Error setting up admin account:${colors.reset}`,
        error,
      );
    }
  } finally {
