...
const checkDatabase = async () => {
  console.log(
    `${colors.blue}Checking database connection and schema status...${colors.reset}`,
  );
  ...
  try {
    ...
    tableResults.forEach((table) => {
      if (table.exists) {
        console.log(
          `${colors.green}✓ Table ${table.name} exists${colors.reset}`,
        );
      } else {
        console.log(
          `${colors.red}✗ Table ${table.name} is missing${colors.reset}`,
        );
        missingTables++;
      }
    });
    ...
    if (missingTables === 0) {
      console.log(
        `${colors.green}✓ All required tables are present${colors.reset}`,
      );
      console.log(
        `${colors.green}✓ Database schema appears to be complete${colors.reset}`,
      );
    ...
    console.error(
      `${colors.red}Error connecting to database:${colors.reset}`,
      error,
    );
    console.log("\nPossible solutions:");
    ...
    console.log(
      "3. Ensure the database exists (run npm run db:clean to create it)",
    );
    process.exit(1);
  }
};
