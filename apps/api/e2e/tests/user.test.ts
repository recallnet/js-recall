...
describe("User API", () => {
  ...
  test("agent handle functionality - auto-generation and validation", async () => {
    // Create a SIWE-authenticated client
    const { client: siweClient } = await createSiweAuthenticatedClient({
      adminApiKey,
  ...
  test("agent handle updates", async () => {
    // Create a SIWE-authenticated client
    const { client: siweClient } = await createSiweAuthenticatedClient({
      adminApiKey,
    ...
    const updateResponse2 = await siweClient.updateUserAgentProfile(agent2Id, {
