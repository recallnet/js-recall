...
// Custom cleanup function to avoid dependency on test-helpers
async function cleanupTestState(): Promise<void> {
  await db.delete(rewards);
  await db.delete(rewardsTree);
  await db.delete(rewardsRoots);
...
describe("Rewards Service", () => {
  ...
  test("allocate method creates merkle tree and stores root hash", async () => {
    ...
    expect(Buffer.from(rootEntries[0]!.rootHash).toString("hex")).toBe("2139ba0bff060eae6c6def620e29a004f4f3c2ea869066dd258ad30959ed16b7");
  ...
  test("allocate method handles single reward correctly", async () => {
    ...
    const address = "0x6813Eb9362372EEF6200f3b1dbC3f819671cBA69" as `0x${string}`;
  ...
  test("retrieveProof method verifies multiple rewards", async () => {
    ...
    const testRewards: InsertReward[] = rewardData.map(({ address, amount }) => {
      const leafHashBuffer = createLeafNode(address, amount);
      return {
        id: crypto.randomUUID(),
        epoch: testEpochId,
    ...
    const fauxLeaf = keccak256(
      encodeAbiParameters(
        ...
        [testEpochId, "0x0000000000000000000000000000000000000000" as `0x${string}`, BigInt("0")]
      )
    );
    
    const leaves = [fauxLeaf, ...rewardEntries.map((reward) => Buffer.from(reward.leafHash))];
    ...
    for (const { address, amount } of rewardData) {
      ...
      const proof = await rewardsService.retrieveProof(testEpochId, address, amount);
      ...
      console.log(`Verified proof for address ${address} with amount ${amount}`);
    }
    
    console.log(`Successfully verified proofs for all ${rewardData.length} rewards`);
  });

  test("retrieveProof method throws error for non-existent reward", async () => {
    // Create a Merkle tree with some rewards first
    const existingAddress = "0x2546BcD3c84621e976D8185a91A922aE77ECEc30" as `0x${string}`;
    ...
    const nonExistentAddress = "0xbDA5747bFD65F08deb54cb465eB87D40e51B197E" as `0x${string}`;
    const nonExistentAmount = BigInt("30");
    
    await expect(
      rewardsService.retrieveProof(testEpochId, nonExistentAddress, nonExistentAmount)
    ).rejects.toThrow(`No proof found for reward (address: ${nonExistentAddress}, amount: ${nonExistentAmount}) in epoch ${testEpochId}`);
  });
});
