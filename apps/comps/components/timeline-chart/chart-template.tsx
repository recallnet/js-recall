...
export const MetricTimelineChart: React.FC<MetricTimelineChartProps> = ({
  ...
  // Show countdown for pending competitions or waiting for data
  const shouldShowStatus =
    status === "pending" || !timelineData || timelineData.length === 0;
  ...
  if (shouldShowStatus) {
    return (
      <ChartStatus
        ...
        boostStartDate={boostStartDate ? new Date(boostStartDate) : null}
        hasData={!!(timelineData && timelineData.length > 0)}
      />
    );
  }
