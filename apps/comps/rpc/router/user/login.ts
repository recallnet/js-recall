...
import { privyUserMiddleware } from "@/rpc/middleware/privy-user";
...
export const login = base
  .use(privyUserMiddleware)
  .handler(async ({ context, errors }) => {
    try {
      // Verify we have a valid Privy identity token
      if (!context.privyIdToken) {
        throw errors.UNAUTHORIZED({ message: "No authentication token found" });
      }

      if (!context.privyUser) {
